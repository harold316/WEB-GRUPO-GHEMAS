...
export function PathologyCard({ patologia }: { patologia: Patologia }) {
  return (
    <Link
      ...
      className="group relative flex h-full min-h-[200px] overflow-hidden rounded-[1.75rem] border border-white/80 bg-white p-5 shadow-[0_8px_30px_rgba(17,124,112,0.08)] transition-all duration-300 hover:-translate-y-1 hover:shadow-lg dark:border-line dark:bg-surface"
    >
      <div className="relative z-10 flex min-w-0 flex-1 flex-col justify-between pr-2">
        ...
        <span className="mt-4 inline-flex min-h-10 items-center gap-1 text-sm font-semibold text-[#117C70] group-hover:text-brand-dark">
          Ver ficha
